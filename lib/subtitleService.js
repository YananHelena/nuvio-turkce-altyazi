...
const SEARCH_TTL_MS = 900_000;
const NEGATIVE_SEARCH_TTL_MS = 120_000;
const LISTING_TTL_MS = 900_000;
const DETAIL_TTL_MS = 21_600_000;
const ARCHIVE_TTL_MS = 21_600_000;
const SUBTITLE_TTL_MS = 86_400_000;
...
function createSubtitleService({ client, logger = console, detailConcurrency = 4 } = {}) {
  ...
  async function getListing(imdbId, type) {
    ...
    return listingFlight.run(key, async () => {
      ...
      const candidates = await client.listCandidates(titlePage, { type, season: null, episode: null });
  ...
  async function findSubtitles({ imdbId, type, season = null, episode = null }) {
    ...
    return searchFlight.run(cacheKey, async () => {
      ...
      const candidates = listing
        .filter((c) => type === 'movie' || (c.season === season && (c.isPackage || c.episode === episode)))
        .slice(0, 50);
        ...
        candidates.map((candidate) =>
          limitDetails(async () => {
            try {
              ...
              logger.warn?.('Subtitle candidate was skipped', { pageUrl: candidate.pageUrl, error: cleanErrorMessage(error) });
              return null;
            }
          })
        )
      ...
      searchCache.set(cacheKey, subtitles, subtitles.length > 0 ? SEARCH_TTL_MS : NEGATIVE_SEARCH_TTL_MS);
      return subtitles;
    });
  }
  ...
  async function getSubtitle({ token, season = null, episode = null }) {
    ...
    return subtitleFlight.run(cacheKey, async () => {
      ...
      const subtitle = await extractSubtitleFromArchive(archive, { season, episode, requireEpisodeMatch: info.isPackage });
...
module.exports = { createSubtitleService };
